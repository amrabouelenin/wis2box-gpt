...
يحتوي wis2box على مجموعة من بيانات المحطات الوصفية التي تُستخدم لوصف المحطات التي تنتج بيانات المراقبة ويجب استرجاعها من **OSCAR/Surface**. تُستخدم بيانات المحطة الوصفية في wis2box بواسطة أدوات تحويل BUFR للتحقق من أن بيانات الإدخال تحتوي على معرف محطة WIGOS (WSI) صالح ولتوفير تعيين بين WSI وبيانات المحطة الوصفية.

## إنشاء رمز تفويض لنقطة النهاية collections/stations

لتعديل المحطات عبر **wis2box-webapp** ستحتاج أولاً إلى إنشاء رمز تفويض.

قم بتسجيل الدخول إلى جهاز الطالب الافتراضي الخاص بك وتأكد من أنك في دليل `wis2box-1.0.0rc1`:
...
على سبيل المثال، لاستخدام رمز تلقائي عشوائي لنقطة النهاية `collections/stations`:
...
يرجى إنشاء رمز تفويض لنقطة النهاية `collections/stations` باستخدام الإرشادات أعلاه.

## إضافة بيانات المحطة الوصفية باستخدام **wis2box-webapp**

يوفر **wis2box-webapp** واجهة مستخدم رسومية لتعديل بيانات المحطة الوصفية.
...
!!! note "إضافة بيانات وصفية للمحطة لثلاث محطات أو أكثر"
    يرجى إضافة ثلاث محطات أو أكثر إلى مجموعة بيانات المحطة الوصفية لـ wis2box الخاص بك.
      
    يرجى استخدام محطات من بلدك إذا أمكن، خاصة إذا كنت قد جلبت بياناتك الخاصة.
      
    إذا لم يكن لديك أي محطات في OSCAR/Surface، يمكنك استخدام المحطات التالية لغرض هذا التمرين:
...
!!! tip "استخراج معلومات الارتفاع المفقودة"
    ...
    على سبيل المثال، للحصول على الارتفاع عند خط العرض -6.15558 وخط الطول 106.84204، يمكنك نسخ ولصق عنوان URL التالي في علامة تبويب متصفح جديدة:
...
تُخزن بيانات المحطة الوصفية في الخلفية لـ wis2box وتتوفر عبر **wis2box-api**.
...
!!! note "تحديث/حذف بيانات المحطة الوصفية"
    ...
    جرب وانظر إذا كنت تستطيع تحديث/حذف بيانات المحطة الوصفية لإحدى المحطات التي أضفتها باستخدام **wis2box-webapp**.

## تحميل بيانات المحطة الوصفية بالجملة

لاحظ أن لدى wis2box أيضًا القدرة على تحميل "بالجملة" لبيانات المحطة الوصفية من ملف CSV باستخدام سطر الأوامر في حاوية **wis2box-management**.
...
هذا يسمح لك بتحميل عدد كبير من المحطات دفعة واحدة وربطها بموضوع معين.
...
بعد إجراء تحميل بالجملة للمحطات، يُوصى بمراجعة المحطات في **wis2box-webapp** للتأكد من أن البيانات تم تحميلها بشكل صحيح.
...
!!! success "تهانينا!"
    ...
    - عرض/تحديث/حذف بيانات المحطة الوصفية باستخدام **wis2box-webapp**