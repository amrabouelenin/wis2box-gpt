...
!!! note
    ...
    Vedi [accessing-your-student-vm](accessing-your-student-vm.md) per istruzioni su come scaricare e installare il pacchetto software wis2box se stai seguendo questa formazione al di fuori di una sessione di formazione locale.

## Preparazione

Accedi alla tua VM designata con il tuo nome utente e password e assicurati di trovarti nella directory `wis2box-1.0.0rc1`:
...
- una directory sulla macchina host da condividere tra la macchina host e i container wis2box definiti dalla variabile d'ambiente `WIS2BOX_HOST_DATADIR`
...
Puoi utilizzare l'opzione di generazione casuale della password quando richiesto per `WIS2BOX_WEBAPP_PASSWORD`, `WIS2BOX_STORAGE_PASSWORD`, `WIS2BOX_BROKER_PASSWORD` e definire le tue.
...
!!! note
    ...
    Il file `wis2box.env` contiene variabili d'ambiente che definiscono la configurazione del tuo wis2box. Per maggiori informazioni consulta la [documentazione di wis2box](https://docs.wis2box.wis.wmo.int/en/latest/reference/configuration.html).
    ...
    Non condividere il contenuto del tuo file `wis2box.env` con nessuno, in quanto contiene informazioni sensibili come le password.
...
```
Nessun file docker-compose.images-*.yml trovato, ne verrà creato uno
Versione corrente=Indefinita, ultima versione=1.0.0rc1
Vuoi aggiornare? (s/n/esci)
```
...
Il download delle immagini potrebbe richiedere del tempo a seconda della velocità della tua connessione internet. Questo passaggio è richiesto solo la prima volta che avvii wis2box.
...
Il wis2box contiene un'API (Interfaccia di Programmazione Applicativa) che fornisce accesso ai dati e processi per la visualizzazione interattiva, la trasformazione dei dati e la pubblicazione.
...
Questa è la pagina di atterraggio dell'API wis2box (eseguita tramite il container **wis2box-api**).
...
Vedrai apparire un pop-up che chiede il tuo nome utente e password. Usa il nome utente predefinito `wis2box-user` e la `WIS2BOX_WEBAPP_PASSWORD` definita nel file `wis2box.env` e clicca su "Accedi":

!!! note 

    Controlla il tuo wis2box.env per il valore della tua WIS2BOX_WEBAPP_PASSWORD. Puoi utilizzare il seguente comando per controllare il valore di questa variabile d'ambiente:
...
Apri MQTT Explorer sul tuo computer e prepara una nuova connessione per connetterti al tuo broker (eseguito tramite il container **wis2box-broker**).
...
Usa i seguenti dettagli di connessione, assicurandoti di sostituire il valore di `<il-tuo-host>` con il tuo hostname e `<WIS2BOX_BROKER_PASSWORD>` con il valore dal tuo file `wis2box.env`:
...
!!! note 
    ...
    Puoi controllare il tuo wis2box.env per il valore della tua WIS2BOX_BROKER_PASSWORD. Puoi utilizzare il seguente comando per controllare il valore di questa variabile d'ambiente:
...
Tieni aperto MQTT Explorer, poiché lo utilizzeremo per monitorare i messaggi pubblicati sul broker.
...
Questa è l'interfaccia utente MinIO (eseguita tramite il container **wis2box-storage**).

Il nome utente e la password sono definiti nel file `wis2box.env` nella tua directory dati wis2box dalle variabili d'ambiente `WIS2BOX_STORAGE_USERNAME` e `WIS2BOX_STORAGE_PASSWORD`. Il nome utente predefinito è `wis2box`.

!!! note 

    Puoi controllare il tuo wis2box.env per il valore della tua WIS2BOX_STORAGE_PASSWORD. Puoi utilizzare il seguente comando per controllare il valore di questa variabile d'ambiente:

    ```{.copy}
    cat ~/wis2box-1.0.0rc1/wis2box.env | grep WIS2BOX_STORAGE_PASSWORD
    ```

    Nota che queste sono le credenziali di lettura/scrittura per la tua istanza MinIO. Non condividere